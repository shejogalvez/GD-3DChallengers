...
export(PackedScene) var item_scene = preload("res://Items/Scenes/Item.tscn")
...
func create_image_material(image : StreamTexture) -> SpatialMaterial:
	...
	#
	$Item/Viewport/TextureRect.texture = image
	# Sets the texture to be the image
	material.albedo_texture = $Item/Viewport.get_texture()
	return material
